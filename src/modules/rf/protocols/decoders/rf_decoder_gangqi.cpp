...
bool rf_decode_gangqi(const std::vector<int>& durations, RfCodes& out) {
    ...
    for (int raw : durations) {
        ...
        switch (step) {
        case ST_RESET:
            if (!level && DURATION_DIFF(dur, TE_LONG * 2) < TE_DELTA * 5) {
                data = 0; bits = 0;
                step = ST_SAVE;
            }
            break;
        ...
        case ST_CHECK:
            if (!level) {
                if (DURATION_DIFF(te_last, (unsigned int)TE_SHORT) < (unsigned int)TE_DELTA &&
                    ...
                    bits++;
                    step = ST_SAVE;
                } else if (DURATION_DIFF(te_last, (unsigned int)TE_LONG) < (unsigned int)TE_DELTA &&
                           DURATION_DIFF(dur, (unsigned int)TE_SHORT) < (unsigned int)TE_DELTA) {
                    data = (data << 1) | 1ULL;
                    bits++;
                    step = ST_SAVE;
                } else if (DURATION_DIFF(dur, TE_LONG * 2) < TE_DELTA * 5) {
                    if (DURATION_DIFF(te_last, (unsigned int)TE_SHORT) < (unsigned int)TE_DELTA)
    ...
    for (int r = 0; r < 5; r++) {
        for (int i = in.Bit - 1; i >= 0; i--) {
            if ((in.key >> i) & 1ULL) {
                ...
                out.push_back(i == 0 ? -(TE_SHORT * 4 + TE_DELTA) : -TE_SHORT);
            } else {
                out.push_back(TE_SHORT);
                out.push_back(i == 0 ? -(TE_SHORT * 4 + TE_DELTA) : -TE_LONG);
            }
        }
    }
    return true;
