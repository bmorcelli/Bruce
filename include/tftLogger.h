...
// Especialização para const char* (evita conversão implícita ruim)
...
void inline buildJson(String &out, const T &key, const V &value, const Rest &...rest) {
    ...
    buildJson(out, rest...); // chamada recursiva
}
...
public:
    void inline setLogging(bool _log = true) {
        ...
        if (_log) log.clear(); // release memory
    };
    ...
    void inline drawArc(
        ...
        if (logging) {
            tftLog l;
            l.function = DRAWARC; // ou defina DRAWARC se quiser adicionar no enum
            l.info = makeJson(
                "x",
                x,
                "y",
    ...
    int16_t inline drawString(const String &string, int32_t x, int32_t y, uint8_t font = 1) {
        if (logging) {
            removeLogEntriesInsideRect(
                x - 1, y - 1, string.length() * LW * textsize + 2, string.length() * LH * textsize + 2
            );
            tftLog l;
            l.function = DRAWSTRING;
            l.info =
                makeJson("x", x, "y", y, "txt", string, "size", textsize, "fg", textcolor, "bg", textbgcolor);
            pushLogIfUnique(l);
            logging = false;
        }
    ...
    int16_t inline drawString(const char *string, int32_t x, int32_t y, uint8_t font = 1) {
        if (logging) {
            removeLogEntriesInsideRect(
                x - 1,
                y - 1,
                (sizeof(string) / 4) * LW * textsize + 2,
                (sizeof(string) / 4) * LH * textsize + 2
            );
            tftLog l;
            l.function = DRAWSTRING;
            l.info =
